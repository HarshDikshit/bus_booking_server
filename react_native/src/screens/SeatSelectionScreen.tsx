import { View, Text, ActivityIndicator, TouchableOpacity, Alert, SafeAreaView, ScrollView } from 'react-native'
...
const SeatSelectionScreen = () => {
    ...
    const bookTicketMutation = useMutation({
        ...
        onError: error=> {
            ...
            Alert.alert(`Failed to book ticket. Please try again. ${error}`)
        },
    });

    const handleSeatSelection = (seat_id: number) => {
        setSelectedSeats(prev => prev.includes(seat_id)? prev.filter(id=> id !== seat_id) : [...prev, seat_id],
    );
    };
